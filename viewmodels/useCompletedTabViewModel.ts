...
export default function useCompletedTabViewModel(persistence: TaskPersistence) {
  ...
  const fetchCompletedTasksFromPersistence = async () => {
    setLoading(true);
    let data: TaskData[] = [];
    try {
      data = await persistence.getCompletedTasks();
    } catch (error) {
      return {
        success: false,
  ...
  return { fetchCompletedTasksFromPersistence, toggleTaskStatus, loading };
}
